...
export class AMF3Bytes extends ByteArray {
    ...
    private encString(value: string, refTables: RefTables, makeHeader?: boolean) {
        ...
        //获取utf8字符串长度
        let strTable = refTables.strTable;
        if (value) {
            //获取utf8字符串长度
            let strTable = refTables.strTable;
            let idx = strTable.indexOf(value);
            if (idx !== -1) {
                this.writeU29(idx << 1);
                return;
            }
    ...
    private encDate(value: Date, refTables: RefTables) {
        ...
        let refTable = refTables.objTable;
        let idx = refTable.indexOf(value);
        if (idx !== -1) {
            this.writeU29(idx << 1);
            return;
        }
    ...
    private writeU29(value: number) {
        ...
        switch (len) {
            case 1:// 0x00000000 - 0x0000007F : 0xxxxxxx
                _buffer.writeUInt8(value, pos);
                break;
            case 2:// 0x00000080 - 0x00003FFF : 1xxxxxxx 0xxxxxxx
                _buffer.writeUInt8(((value >> 7) & 0x7F) | 0x80, pos);
                _buffer.writeUInt8(value & 0x7F, pos + 1);
                break;
            case 3:// 0x00004000 - 0x001FFFFF : 1xxxxxxx 1xxxxxxx 0xxxxxxx
                _buffer.writeUInt8(((value >> 14) & 0x7F) | 0x80, pos);
                _buffer.writeUInt8(((value >> 7) & 0x7F) | 0x80, pos + 1);
                _buffer.writeUInt8(value & 0x7F, pos + 2);
                break;
            case 4:// 0x00200000 - 0x3FFFFFFF : 1xxxxxxx 1xxxxxxx 1xxxxxxx xxxxxxxx
                _buffer.writeUInt8(((value >> 22) & 0x7F) | 0x80, pos);
                _buffer.writeUInt8(((value >> 15) & 0x7F) | 0x80, pos + 1);
                _buffer.writeUInt8(((value >> 8) & 0x7F) | 0x80, pos + 2);
                _buffer.writeUInt8(value & 0xFF, pos + 3);
                break;
        }
    ...
    public readObject(refTables?: RefTables) {
        ...
        let type = this.data.readUInt8(this._position++);
        if (!refTables) refTables = new RefTables();
        switch (type) {
    ...
    private readU29() {
        if (!this.validate(ByteArraySize.SIZE_OF_INT8)) return null;
        let value = this.data.readUInt8(this._position++);

        if (value < 0x80) {//U29 1bytes 0x00-0x7f
            return value;
        }
        if (!this.validate(ByteArraySize.SIZE_OF_INT8)) return null;
        let tmp = this.data.readUInt8(this._position++);
        value = (value & 0x7f) << 7;
    ...
    private readStringAMF3(refTable: RefTables) {
        ...
        if (inline) {
            ...
            let str = this.data.toString("utf8", start, this.position);
    ...
    private readObj(handle: number, refTable: RefTables) {
        let traits: string[], classDef: ClassDefine, className, len, i, isExternalizable, isDynamic, instance;
        let inlineClassDef = (handle & 1) == 0;
        handle >>= 1;
        if (inlineClassDef) {
        ...
        if (isExternalizable) {
            let ref = AMF3Bytes.extDic[className];
            if (ref) {
                instance = new ref();
    ...
    private readDate(refTable: RefTables) {
        let date = new Date(this.readDouble());
    ...
    private readXML(length: number, refTable: RefTables) {
        ...
        let start = this.position;
        this.position += length;
        let xmlString = this.data.toString("utf8", start, this.position);
    ...
    private _readByteArray(length: number, refTable: RefTables) {
        ...
        let start = this.position;
        this.position += length;
        let buf = this.data.slice(start, this.position);
    ...
    private readDictionary(length: number, refTable: RefTables) {
        ...
        for (let i = 0; i < length; i++) {
            let key = this.readObject(refTable);
            let value = this.readObject(refTable);
            dic.set(key, value);
        }
        return dic;
    }
