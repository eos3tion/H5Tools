        ...
        ProtoBuf.DotProto = (function (ProtoBuf, Lang) {
            ...
            ParserPrototype._parseMessage = function (parent, fld) {
                ...
                var msg = {
                    ...
                    "services": [],
                    "oneofs": {},
                    "comment": this.frontComment || "", // 消息前面写的注释
                    "source": undefined //源码
                ...
                while ((token = this.tn.next()) !== '}') {
                    if (Lang.RULE.test(token))
                        ...
                        if (!this.proto3)
                            throw Error("illegal field rule: " + token + ",错误的内容为:" + msg.name);
                        this._parseMessageField(msg, "optional", token);
                    } else
                        throw Error("illegal message token: " + token);
                }
                var endIdx = this.tn.index;
                msg.source = "message" + this.tn.source.substring(startIdx, endIdx);
                this.tn.omit(";");
            ...
            ParserPrototype._parseMessageField = function (msg, rule, type) {
                ...
                if (rule === "map") {
                    ...
                    if (type === "group") {
                        ...
                        token = this.tn.next();
                        if (!Lang.NAME.test(token))
                            throw Error("illegal message field name: " + token + "," + type);
                        fld["name"] = token;
                        this.tn.skip("=");
                        fld["id"] = mkId(this.tn.next());
                        token = this.tn.peek();
                ...
                // 检查后注释
                this._parseComment(fld);
    
                return fld;
            };
    
            /**
             * 解析注释 并将注释赋值于param的comment
             * @param {Object} fld
             * private
             */
            ParserPrototype._parseComment = function (fld) {
                var tn = this.tn;
                var idx = tn.index;
